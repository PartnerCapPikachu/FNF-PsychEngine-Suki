...
call haxelib install tink_core          2.1.1  --quiet --always --skip-dependencies
...
call :installGit funkin.vis       https://github.com/FunkinCrew/funkin.vis
...
if exist ".haxelib\%~1" (
	echo Cleaning existing .haxelib\%~1 ...
	attrib -r -s -h ".haxelib\%~1\*.*" /s /d >nul 2>&1
	rmdir /s /q ".haxelib\%~1"
)
